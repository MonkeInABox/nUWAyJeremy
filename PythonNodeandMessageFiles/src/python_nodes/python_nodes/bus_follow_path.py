...
from nav2_msgs.action import NavigateThroughPoses, NavigateToPose, FollowWaypoints, FollowPath, ComputePathToPose, ComputePathThroughPoses
from nav2_msgs.srv import LoadMap, ClearEntireCostmap, ManageLifecycleNodes, GetCostmap
from geometry_msgs.msg import PoseStamped # Pose with ref frame and timestamp
from nav_msgs.msg import Path,Odometry # Pose with ref frame and timestamp
...
'''
Navigates a robot through path.
'''
...
class BasicNavigator(Node):
    ...
    def followPath(self, path):
        # Sends a `FollowPath` action request
        ...
        self.info('Following ' + str(len(goal_msg.path.poses)) + ' path points.' + '...')
        ...
        if not self.goal_handle.accepted:
            self.error('Following ' + str(len(path.poses)) + '  path points request was rejected!')
            return False
    ...
    def waitUntilNav2Active(self):
        ...
        self._waitForNodeToActivate('map_server')
    ...
    def debug(self, msg):
        ...
        return
class MinSubPub(Node):
  
  def __init__(self):
    super().__init__('bus_follow_path_sub')
    ...
    #self.timer = self.create_timer(timer_period, self.pub_path_cb)
    self.subscription
    self.x = 0.0
    self.y = 0.0
    self.z = 0.0
...
def main():
  ...
  min_subpub = MinSubPub()

  rclpy.spin_once(min_subpub)
...
#   with open('/home/quirky/ultimateGuide/dev_ws/src/two_wheeled_robot/params/coordinates.csv', newline = '') as f:
  ...
  order_waypoints.sort(key = lambda p: (p.pose.position.x - min_subpub.x)**2 + (p.pose.position.y - min_subpub.y)**2)
  ...
  goal_path.header.stamp = navigator.get_clock().now().to_msg()
  goal_path.poses=goal_poses
  #lan_publisher = rclpy.create_publisher(Path, '/plan', 1)
  min_subpub.plan_publisher.publish(goal_path)
  navigator.followPath(goal_path)
  ...
  while not navigator.isNavComplete():
    ...
    if feedback and i % 5 == 0:
      print('Distance to goal: ' + '{:.2f}'.format(
