"""
ROS2 node that subscribes to the front/rear LiDAR PointCloud2 topics, runs
them through the trained PointCAM shuttle-bus model, and publishes the
predicted speed/steering as a cmd_vel Twist.
 
This is the inference-time counterpart of shuttleBusTrainCAM.py: it
replicates the exact same preprocessing (LiDAR extrinsic transform, crop
bounds, zero-padding to a fixed point count, then FPS downsampling) so the
model sees data shaped the same way it was trained on.
 
Run with:
    ros2 run <your_package> lidarCmdVelPublisher.py --ros-args \
        -p checkpoint_path:=/path/to/pointcam_shuttle_lane_following.pth
 
or plain:
    python3 lidarCmdVelPublisher.py
(edit the parameter defaults below, or override via --ros-args -p name:=value)
"""
...
# --- Same LiDAR extrinsics / crop bounds / padding used in shuttleBusTrainCAM.py -----
...
# Sanity clip on published commands -- matches the corrupt-data filter used
# when the training data itself was collected (see convertLidarDataNew.py).
MAX_ABS_SPEED = 5.5
MAX_ABS_STEER = 0.32
...
def preprocess_cloud(msg: PointCloud2, transformation_matrix: np.ndarray) -> np.ndarray:
    """
    PointCloud2 -> cropped/transformed (M, 3) xyz array, then zero-padded (or
    randomly subsampled, if it's somehow larger than PADDING) to exactly
    PADDING points. Mirrors load_latest_data()/dataset.__getitem__() in
    shuttleBusTrainCAM.py, minus the training-time random scale/translate
    augmentation (we want deterministic inference, not augmented inputs).
    """
    points = structured_to_unstructured(
        point_cloud2.read_points(msg, field_names=("x", "y", "z", "intensity"), skip_nans=True)
    )
    points = np.delete(points, -1, axis=1)  # drop intensity -> (N, 3)
 
    ones = np.ones((points.shape[0], 1))
    points_h = np.hstack([points, ones])  # (N, 4)
    points_h = (transformation_matrix @ points_h.T).T  # (N, 4)
 
    px, py, pz = points_h[:, 0], points_h[:, 1], points_h[:, 2]
    mask = (px > x_min) & (px < x_max) & (np.abs(py) < y_limit) & (pz < z_limit)
    cropped = points_h[mask, :3]  # (M, 3)
...
class LidarCmdVelPublisher(Node):
    def __init__(self):
        ...
        self.declare_parameter("sync_slop_sec", 0.1)  # front/rear timestamp tolerance for pairing
        ...
        # checkpoint_path=None here -- we build the PointCAM encoder from
        # scratch (untrained) and then immediately overwrite every weight
        # (encoder + heads) from your trained shuttle-bus checkpoint below.
        ...
        # Training checkpoints saved mid-run are dicts with 'model_state_dict';
        # the final save (torch.save(model.state_dict(), ...)) is the raw
        # state dict itself. Handle both.
    ...
    def lidar_callback(self, front_msg: PointCloud2, rear_msg: PointCloud2):
        # Frozen-sensor guard: if we're seeing the exact same front-cloud
        # timestamp as last callback, don't drive on stale data.
        ...
        # Same corrupt/out-of-range guard used when the training data was collected.
