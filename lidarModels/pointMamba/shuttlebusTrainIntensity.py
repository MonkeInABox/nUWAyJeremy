...
from modelIntensity import PointMamba
...
class dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if front_lidar.size(1) < padding:
            filler = torch.full((1, padding - front_lidar.size(1), 4), PAD_VALUE)
            front_lidar = torch.cat((front_lidar, filler), 1)
        front_lidar = torch.squeeze(front_lidar)

        rear_lidar = self.transform(rear_lidar)
        if rear_lidar.size(1) < padding:
            filler = torch.full((1, padding - rear_lidar.size(1), 4), PAD_VALUE)
...
def load_latest_data(Search_Folder):
    ...
    Speed = data[3][0]
    Steering_Angle = data[4][0]
    front_lidar = data[0][:, :3]
    front_lidar_intensity = data[0][:, 3:]
    rear_lidar = data[1][:, :3]
    rear_lidar_intensity = data[1][:, 3:]
    ...
    front_lidar_intensity = front_lidar_intensity[front_mask]
    rear_cropped_points = rear_lidar[rear_mask, :3]
    rear_lidar_intensity = rear_lidar_intensity[rear_mask]


    front_cropped_points = np.hstack([front_cropped_points, front_lidar_intensity])
    rear_cropped_points = np.hstack([rear_cropped_points, rear_lidar_intensity])

    front_cropped_points = normalize_pointcloud(front_cropped_points)
    rear_cropped_points  = normalize_pointcloud(rear_cropped_points)
...
def random_sample_points(points, target_count):
    """Randomly sample points if there are more than target_count."""
    B, N, C = points.shape
    if N > target_count:
        idx = torch.randperm(N, device=points.device)[:target_count]
        points = points[:, idx, :]
    return points
...
def normalize_pointcloud(pc):
    ...
    xyz = pc[:, :3]
    intensity = pc[:, 3:]
    xyz = xyz / 20.0  # assuming 20 is the intended max distance
...
if __name__ == "__main__":
    ...
    model_path_name = "pointMambaIntensity_shuttle_lane_following_17_0.0015_0.6351_0.7435.pth"
    checkpoint = None
    ...
    Base_Path = dirname("/media/quirky/EglintonData/lidar_data_sorting/LiDAR_Dual_Stage2/")
    ...
    for Folders in os.listdir(Base_Path):
        ...
        for name in model_files:
            if name == Folders:
                Lidar_Paths.append(join(Base_Path, Folders))

    
    for Lidar_Path in Lidar_Paths:
        if exists(Lidar_Path):
            for Folders in os.listdir(Lidar_Path):
                Search_Folder = join(Lidar_Path, Folders)
                # i = 0
                # past_data.clear()
    ...
    scheduler = ReduceLROnPlateau(optimizer, 'min', factor=gamma, patience=3, threshold=1e-5, threshold_mode='abs', verbose=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        while len(temp_search) != 0:
            ...
            while (len(Front_lidar_all) < 300) and len(temp_search) != 0: #30000
        ...
        # for data1, data2, label1, label2 in train_loader:
            for data1, data2, label1, label2 in train_loader:
                ...
                # Random sample first
                data1 = random_sample_points(data1, points_all)
                data2 = random_sample_points(data2, points_all)

                # Then FPS to npoints
                fps_idx = pointnet2_utils.furthest_point_sample(data1, npoints)
                data1 = pointnet2_utils.gather_operation(data1.transpose(1, 2).contiguous(), fps_idx).transpose(1, 2).contiguous()

                fps_idx = pointnet2_utils.furthest_point_sample(data2, npoints)
                ...
                print(data2.min(), data2.max(), data2.mean())
        ...
        if best_loss > total_epoch_loss:
            ...
            save_name = f"pointMambaIntensity_shuttle_{model_name}_{epoch+1}_{total_epoch_loss:.4f}_{total_epoch_accuracy1:.4f}_{total_epoch_accuracy2:.4f}.pth"
    ...
    torch.save(model.state_dict(), f'pointMambaIntensity_shuttle_{model_name}.pth')
    writer.flush()
    writer.close()
