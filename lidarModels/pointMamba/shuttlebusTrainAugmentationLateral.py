...
class drivePointMamba(nn.Module):
    ...
    def forward(self, front_cloud, rear_cloud, delta_y):
        ...
        if delta_y != 0:
            t = torch.tensor([0.0, -delta_y, 0.0], device=full_lidar.device)
            R = torch.eye(3, device=full_lidar.device)

            full_lidar = full_lidar @ R.T + t
...
class dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        front_lidar = torch.from_numpy(front_lidar).float().unsqueeze(0)
        rear_lidar  = torch.from_numpy(rear_lidar).float().unsqueeze(0)
        ...
        # delta_y = (torch.rand(1).item() * 2 - 1) * max_shift
        # t = torch.tensor([0.0, -delta_y, 0.0], device=front_lidar.device)
        # R = torch.eye(3, device=front_lidar.device)

        # front_lidar = front_lidar @ R.T + t
        # rear_lidar = rear_lidar @ R.T - t

        # label2 = label2 + (delta_y / max_shift) * 0.2
...
if __name__ == "__main__":
    ...
    model_path_name = "pointMambaAugmented_shuttle_lane_following_50_0.0011_0.7062_0.7996.pth"
    checkpoint = None
    ...
    if args.load_model:
        ...
        for param_group in optimizer.param_groups:
            ...
            new_lr = current_lr * 1.0
            param_group['lr'] = new_lr
            print(f"LR updated: {current_lr:.8f} → {new_lr:.8f}")
    # scheduler = StepLR(optimizer, step_size=10, gamma=gamma)
    scheduler = ReduceLROnPlateau(optimizer, 'min', factor=gamma, patience=2, threshold=0.0001, threshold_mode='abs', verbose=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        while len(temp_search) != 0:
            ...
            train_data = dataset(Front_clouds_Train, Rear_clouds_Train, Speed_Train, Steering_Angle_Train, transform=training_transform)
            test_data = dataset(Front_clouds_Test, Rear_clouds_Test, Speed_Test, Steering_Angle_Test, transform=None)
            val_data = dataset(Front_clouds_Valid, Rear_clouds_Valid, Speed_Valid, Steering_Angle_Valid, transform=None)
        ...
        # for data1, data2, label1, label2 in train_loader:
            ...
            # for i in range (0, len(Image_Train)):
                ...
                output1, output2 = model(data1, data2, delta_y)
                # print(output1.shape)
                # print(label1.shape)
            ...
            with torch.no_grad():
                ...
                for data1, data2, label1, label2 in val_loader:
                    ...
                    val_output1, val_output2 = model(data1, data2, delta_y)
        ...
        if best_loss > total_epoch_loss:
            ...
            save_name = f"pointMambaAugmentedFullLateral_shuttle_{model_name}_{epoch+1}_{total_epoch_loss:.4f}_{total_epoch_accuracy1:.4f}_{total_epoch_accuracy2:.4f}.pth"
    ...
    torch.save(model.state_dict(), f'pointMambaAugmentedFullLateral_shuttle_{model_name}.pth')
    writer.flush()
    writer.close()
