...
class Group(nn.Module):
    ...
    def _fps(self, xyz, num_group):
        # Replaces the original misc.fps(xyz, num_group), which required
        # building the pointnet2_ops CUDA extension. pytorch3d's
        # sample_farthest_points does the same job (farthest-point-sample
        # num_group centers from xyz) without a second CUDA build -- this
        # is the same substitution already used for this pipeline's outer
        # FPS downsampling step, kept consistent here.
        centers, _ = sample_farthest_points(xyz, K=num_group, random_start_point=True)
        return centers
