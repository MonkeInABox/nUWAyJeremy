...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        width = conv1d_weight.size(-1)
        if dx is None:
            dx = torch.empty_like(x)
        dconv1d_weight = torch.zeros_like(conv1d_weight, dtype=torch.float32)
        dconv1d_bias = None
        if conv1d_bias is not None:
            dconv1d_bias = torch.zeros_like(conv1d_bias, dtype=torch.float32)
        ...
        causal_conv1d_cuda.causal_conv1d_bwd(
            x, conv1d_weight, conv1d_bias, dconv1d_out, None, None, None, dx, dconv1d_weight, dconv1d_bias, dinitial_states, True
        )
...
class BiMambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias,None, None, conv1d_out, None, True)
        # We're being very careful here about the layout, to avoid extra transposes.
        # We want delta to have d as the slowest moving dimension
        # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
        x_dbl = F.linear(rearrange(conv1d_out, 'b d l -> (b l) d'), x_proj_weight)  # (bl d)
